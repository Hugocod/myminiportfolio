...
function callback(entries, observer) {
  entries.forEach(function (entry) {
    if (entry.isIntersecting && entry.target.classList.contains("target-1")) {
      movingLetters("text-1", "sp-1"); // Do this when the observer see the target
      document
        .getElementById("text-1")
        .lastElementChild.insertAdjacentHTML(`afterend`, svgHi);
      // observer.unobserve(entry.target); // Stop observing this target
    } else if (
      entry.isIntersecting &&
      entry.target.classList.contains("target-2")
    ) {
      movingLetters("text-2", "sp-2"); // Do this when the observer see the target
      observer.unobserve(entry.target); // Stop observing this target
    } else if (
      entry.isIntersecting &&
      entry.target.classList.contains("target-3")
    ) {
      movingLetters("text-3", "sp-3"); // Do this when the observer see the target
      observer.unobserve(entry.target); // Stop observing this target
    } else if (
      entry.isIntersecting &&
      entry.target.classList.contains("target-4")
    ) {
      movingLetters("text-4", "sp-4"); // Do this when the observer see the target
      observer.unobserve(entry.target); // Stop observing this target
    }
  }); // forEach ENDpoint
} // Callback ENDpoint
...
function movingLetters(id, spanClass) {
  let animatedText = document.getElementById(id);
  animatedText.innerHTML = animatedText.textContent.replace(
    /\S/g,
    `<span class='${spanClass} iniziale '>$&</span>`
  );

  anima();

  // let lettersGroup = Array.from(document.getElementsByClassName(spanClass));

  // lettersGroup.forEach((lettera, i) => {
  //   function appari() {
  //     lettera.classList.replace("iniziale", "finale");
  //   }

  //   setTimeout(appari, i * 20);
  // });
}
